...
#if defined(NDEBUG)
# error "Iwashicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1fcdbf07c6e15144583ec6469884d85cb69715529a788636ff97d12ab55e1ed8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Iwashicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Iwashicoin Signed Message:\n";
...
{
    // Iwashicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Iwashicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Iwashicoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Iwashicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Iwashicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Iwashicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 12/May/2018 Charlie Russell, Who Befriended Bears, Dies at 76";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a56e3a20ce41b37d3cfc0bc0c8edaa200136daba8565d7d49b388f2d03e9af25068bed5d6ead2edbb19504805a6343a00a5188770f6d4c869657f60127e369a2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526193901;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086437309;

        if (fTestNet)
        {
            block.nTime    = 1526203920;
	...
	// ------
        ...
        assert(block.hashMerkleRoot == uint256("0x19371b2736cbba37779c5ce02877bfa986bc181265ac1237abf15cab5ae5da63"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xb3, 0xd3 }; // Iwashicoin: increase each by adding 2 to bitcoin's value.
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTEIWS_BLOCK)
            {
                bool send = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IwashicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IwashicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IwashicoinMiner(CWallet *pwallet)
{
    printf("IwashicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("iwashicoin-miner");
    ...
    try { loop {
        ...
        printf("Running IwashicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IwashicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IwashicoinMiner, pwallet));
}
